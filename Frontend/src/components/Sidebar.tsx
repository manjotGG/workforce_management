...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const menuItems = [
    ...
    { id: 'salary', label: 'Payroll & Salary', icon: BadgeIndianRupee },
    ...
    <aside className="w-64 bg-slate-900/90 border-r border-slate-800/80 flex flex-col justify-between min-h-screen sticky top-0 z-30 shadow-2xl backdrop-blur-xl">
      ...
      <div className="p-4 m-3 rounded-2xl bg-slate-850/80 border border-slate-800/80 flex items-center justify-between">
        <div className="flex items-center gap-3 overflow-hidden">
          <div className="w-9 h-9 rounded-full bg-indigo-600/30 border border-indigo-500/40 flex items-center justify-center text-indigo-300 font-semibold text-sm">
          ...
          <div className="overflow-hidden">
            ...
            <p className="text-xs text-slate-400 capitalize truncate">
              {currentUser?.role?.toLowerCase().replace('_', ' ') || 'Administrator'}
            </p>
          </div>
        </div>
        <button
